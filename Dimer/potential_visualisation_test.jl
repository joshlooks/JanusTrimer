...
const ab = 0.05 # Hydrophillic - Hydrophobic
const bb = 0.0025 # Hydrophobic - Hydrophobic
const aw = 1.00 # Hydrophillic - Solvent
const bw = 0.05 # Hydrophobic - Solvent
...
# global theta1s = range(0, stop=π/2, length=19)
# global theta2s = range(π/2, stop=π, length=19)

# theta1 = theta1s[Int(round(task_index/19, RoundDown)) + 1]
# theta2 = theta2s[task_index % 19 + 1]

global theta1 = 80.0/180*π
global theta2 = (180.0-80.0)/180*π
...
global partition_angle = π / 2
# global phi = (phi_chooser / 9) * π

const β = 2R + ds
const β2 = β^2

global point_location = Float64[12, 0, β]

const translate2 = Float64[0, 0, β]

const maxevals1 = 1000000
const atol1 = 1e-8

# This function is for the integrand of each half of the particles
# interacting with another half of the other particle
# [theta1, phi1, theta2, phi2]
function point_sphere1_integrand(x)::Float64
    spherical1 = Float64[R, x[1], x[2]]

    r2 = sqeuclidean(convert_to_cartesian_angle(spherical1, angle_offset1_data), point_location)

    potential_distance_squared(σ2, r2) * R2 * sin(x[1])
end

function point_sphere2_integrand(x)::Float64
    spherical2 = Float64[R, x[1], x[2]]

    r2 = sqeuclidean(convert_to_cartesian_angle(spherical2, angle_offset2_data) + translate2, point_location)
...
function e1()
    e = hcubature(point_sphere1_integrand, Float64[0, 0], Float64[partition_angle, 2π], maxevals = maxevals1, atol = atol1)
    # println(e[1], " ", e[2])
    surfaceeta * 4 * ε * bw * e[1]
end

# Interaction between Hydrophillic and Hydrophobic
function e2()
    e = hcubature(point_sphere1_integrand, Float64[partition_angle, 0], Float64[π, 2π], maxevals = maxevals1, atol = atol1)
    # println(e[1], " ", e[2])
    surfaceeta * 4 * ε * aw * e[1]
end

# Interaction between Hydrophillic and Hydrophobic
function e3()
    e = hcubature(point_sphere2_integrand, Float64[0, 0], Float64[partition_angle, 2π], maxevals = maxevals1, atol = atol1)
    # println(e[1], " ", e[2])
    surfaceeta * 4 * ε * bw * e[1]
end
...
function e4()
    e = hcubature(point_sphere2_integrand, Float64[partition_angle, 0], Float64[π, 2π], maxevals = maxevals1, atol = atol1)
    # println(e[1], " ", e[2])
    surfaceeta * 4 * ε * aw * e[1]
end
...
# [r2, theta2, phi2]
function point_sphere_fluid(x)::Float64
...
#     surfaceeta * volumeeta * 4 * ε * bw * e[1] * surface_area
# end
...
#     surfaceeta * volumeeta * 4 * ε * aw * e[1] * surface_area
# end
...
#     global theta1 = theta1s[Int(round(task_index/19, RoundDown)) + 1]
#     global theta2 = theta2s[task_index % 19 + 1]
...
#     global angle_offset1_data = create_angle_offset_data(angle_offset1)
#     global angle_offset2_data = create_angle_offset_data(angle_offset2)
    x_loc = (task_index % 201) / 200 * 4 * R - 2 * R
    z_loc = (task_index ÷ 201) / 400 * (6 * R + ds) - 2 * R

    global point_location = Float64[x_loc, 0, z_loc]

    # print(point_location)

    if sqeuclidean(point_location, [0.0, 0.0, 0.0]) < Rdw2
        continue
    end

    if sqeuclidean(point_location, translate2) < Rdw2
        continue
    end
...
#     println("E10: ", c10, " time: ", t10 - t9, "s")
    ...
    et = c1 + c2 + c3 + c4
    println(task_index, " ET: ", et, " time: ", t4 - tstart, "s")


    s = string(x_loc, ",", z_loc, ",", c1, ",", c2, ",", c3, ",", c4, ",", et, "\n")
    global output = string(output, s)
end

# file = string("/nesi/nobackup/uoa00623/Jack/vary_patch/J", angle_chooser + 1, "0_P", 2 * phi_chooser, "0.csv")
# # # file = "/nesi/nobackup/uoa00623/Jack/integration_S051_W080_R10_P0_A025.csv"
file = "potential_80_80_R6_HR_080.csv"

# # append the csv columns to the data
if !isfile(file)
    global output = string("x,z,e1,e2,e3,e4,energy\n", output)
end
