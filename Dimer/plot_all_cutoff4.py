...
different_df = [0.51, 0.41, 0.61, 0.71, 0.81, 0.91]
...
for (j, filename) in enumerate(files_grouped):
    ...
    plt.plot(theta12s, 100 * test3 / np.sum(test3), marker='o', linestyle='dashed', linewidth=2, markersize=4, label=different_df[j])
...
axs.set_xlabel(r"$\theta_i, \theta_j$")
axs.set_ylabel(r"probability [%]")
axs.xaxis.set_ticks(np.arange(0, 90.01, 10))
