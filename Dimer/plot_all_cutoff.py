...
for (j, filename) in enumerate(files_grouped):
    ...
    axs[0].errorbar(different_radius[j], theta12s[cutoff_index], yerr=5, color="tab:blue", marker="o", markersize = 4)
...
axs[0].plot(values, formula, color="black", linestyle="dashed")
...
files += glob.glob("data/vary_patch/J30*.csv")
files += glob.glob("data/vary_patch/J20*.csv")
...
files_grouped = [files[10*i:10*i+10] for i in range(7)]
...
for filename in files_grouped:
    ...
    # print(int(filename[0].split("\\")[1].split("_")[0][1:]))

    # axs[0].scatter(int(filename[0].split("\\")[1].split("_")[0][1:]), theta12s[cutoff_index], color="tab:blue", marker="o")
    # axs[1].scatter(int(filename[0].split("\\")[1].split("_")[0][1:]), theta12s[np.argmax(test3)], color="tab:blue", marker="o")

    axs[1].errorbar(int(filename[0].split("\\")[1].split("_")[0][1:]), theta12s[cutoff_index], yerr=5, color="tab:blue", marker="o", markersize = 4)
    # axs[1].errorbar(int(filename[0].split("\\")[1].split("_")[0][1:]), theta12s[np.argmax(test3)], yerr=5, color="tab:blue", marker="o")

    # print(str(int(filename[0].split('\\')[1].split('_')[0][1:])) + ","  + str(theta12s[cutoff_index]))
    # print(str(int(filename[0].split('\\')[1].split('_')[0][1:])) + ","  + str(theta12s[np.argmax(test3)]))
...
values = np.linspace(0.0, 120.0, 1000)

formula = values - math.degrees(math.acos((10.0 + 0.51/2.0) / (10.0 + 0.8)))

axs[1].plot(values, formula, color="black", linestyle="dashed")

# formula = 90 * np.square(np.sin(np.radians(values) / 2))
...
# axs[0].set_ylabel(r"cutoff angle")
# axs[0].xaxis.set_ticks(np.arange(0, 120.01, 10))
# axs[0].yaxis.set_ticks(np.arange(0, 90.01, 15))
# axs[0].set_xlim([0, 100])
# axs[0].set_ylim([0, 90])


axs[0].set_xlabel(r"$\it{R}(\sigma)$")
axs[0].set_ylabel(r"Cutoff angle (degrees)")
axs[0].xaxis.set_ticks(np.arange(0, 80.01, 10))
axs[0].yaxis.set_ticks(np.arange(0, 90.01, 15))
axs[0].set_xlim([0, 80])
axs[0].set_ylim([0, 90])


axs[1].set_xlabel(r"$\gamma (degrees)$")
axs[1].set_ylabel(r"Cutoff angle (degrees)")
axs[1].xaxis.set_ticks(np.arange(0, 120.01, 10))
...
plt.tight_layout()
plt.savefig("plots/figure10.png")
